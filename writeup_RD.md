...
[video1]: ./ouput_videos/project_video_output.mp4 "Video"

Instructions: 

1. Execute: python CameraCalibration.py

2. Execute: python Improved_ImageProcessing.py 
...
### Camera Calibration (OK)

#### 1. Briefly state how you computed the camera matrix and distortion coefficients. Provide an example of a distortion corrected calibration image. (OK)
...
I even implemented an validation of the camera calibration in order to see if you can use the calibration data for undistortion the picture.

I used the repjojection error to do the validation, which indicates how good you can send a real world point through the camera matrix into the 2d image point.

Then the same is done through the invers of the camera matrix to project the image point back to the real world coordinates. The eror result is given in pixels.

*My levels in the validation are:*

result >1.0 pixel:                Camera calibration can not be used, redo the taking picture step

1.0 pixel > result > 0.1 pixel:   Camera calibration is good for use.  

result < 0.1 pixel:               Camera Calibration is almost perfect

### Pipeline (single images)

#### 1. Provide an example of a distortion-corrected image. (OK)
...
I explored several combinations of sobel gradient thresholds and color channel thresholds in multiple color spaces. These are labeled clearly in the Improved_ImageProcessing.py file. 

- image preprocessing 

- Determine the mask for defining the area where to look for the lane lines

- perform a perspective transform of the detected line are in order to determine if the lines are parallel and to be able to calculate the correct curvature value

- do a histogram analysis to finde the best starting points for the sliding window functions

- do a polyfit

- calculate the  curvature of the lanes

- draw the detected lanes back onto the original image.
...
#### 3. Describe how (and identify where in your code) you performed a perspective transform and provide an example of a transformed image. (TBD)

#Improved_ImageProcessing.py line 95- have to see when the cell ends...152?
The code for my perspective transform is titled "Perspective Transform" in the code.  The `unwarp()` function takes as inputs an image (`img`), as well as source (`src`) and destination (`dst`) points.  I chose to hardcode the source and destination points in the following manner:
...
The main function for calculating the values is called in this line in 'Improved_ImageProcessing.py':
``` 
rad_l, rad_r, d_center = calc_curv_rad_and_center_dist(exampleImg_bin, left_fit, right_fit, left_lane_inds, right_lane_inds)
```

#### 6. Provide an example image of your result plotted back down onto the road such that the lane area is identified clearly. (TBD)


I implemented this step in the code cells titled "Draw the Detected Lane Back onto the Original Image" (Improved_ImageProcessing.py,line 606 ff) and "Draw Curvature Radius and Distance from Center Data onto the Original Image"(Improved_ImageProcessing.py, line TBD ff) in the Jupyter notebook. A polygon is generated based on plots of the left and right fits, warped back to the perspective of the original image using the inverse perspective matrix `Minv` and overlaid onto the original image. The image below is an example of the results of the `draw_lane` function:
...
[Project video - performing good](./project_video_output_diag.mp4)

[Challenge video - performing ok, but certain blank outs and misdetections](./challenge_video_output_diag.mp4)

[Harder challenge video - not really good](./harder_challenge_video_output_diag.mp4)
...
improvement of the image processing can also be obtainted by normalizing the brightness level. It is not implemented. Also Gradient analysis can ensure better contrasts.

*Addendum: Below are diagnostic versions of the output for each of the videos included in the project.*

[Project video](./project_video_output_diag.mp4)

[Challenge video](./challenge_video_output_diag.mp4)

[Harder challenge video](./harder_challenge_video_output_diag.mp4)